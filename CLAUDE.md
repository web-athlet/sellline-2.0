...
70. **[Done Session 17] TD-S16b-01** — Workspace-Packages (`@nextgen/db|types|utils`) auf CJS-Output umgestellt (Precompile statt TS-Source-Export); `turbo.json` `dev`-Task auf `dependsOn: ["^build"]` gesetzt (Packages werden vor App-Dev gebaut); `dotenv-cli` für env-Loading im `dev`-Script. `node dist/main.js` im API-Prod-Image kann die Packages jetzt sauber `require`en — Source-Stopgap im `apps/api/Dockerfile` entfällt.
