...
| [session-16a-summary.md](20-sessions/session-16a-summary.md) | completed | Testing & Performance (Foundation-Scope): testcontainers-Integration-Harness (pgvector + Redis, Migrationen via `migrate deploy` → TD#53 für Test-Pfad gelöst), Fishery-Test-Daten-Factories (9 Entities), deterministische WS-Integration-Tests, parallele CI-Jobs (lint/typecheck/unit/integration/build) + main-only E2E-Workflow. Playwright (5 Happy-Paths × 3 Projects) + k6 (3 Skripte) als Scaffold (nicht ausgeführt). 4 Integration-Suites / 10 Tests. Quality-Gate 10/10 (Integration-Check jetzt real). Neue Deps: fishery, faker, testcontainers@10, socket.io-client, express@4 (api), @playwright/test (root). |
| [session-16b-summary.md](20-sessions/session-16b-summary.md) | completed | PWA & CI/CD (Production-Launch-Finalisierung): `@ducanh2912/next-pwa` (NetworkFirst API + CacheFirst Bilder/static, `skipWaiting:false`) + Manifest + 8 Icons (sharp) + expliziter `PWAUpdatePrompt` (kein Silent-Update). Health-Liveness/Readiness-Split (`/api/health/ready` → 503 bei DB-Fehler). RedisService-Shutdown-Härtung (TD-S16a-02 gelöst). Multi-Stage-Dockerfiles (web+api, Monorepo-Standalone) + `deploy.yml` (GHCR → kubectl, prod-Approval; ci.yml/e2e.yml unverändert) + vollständige k8s-Manifeste (EU-only, RollingUpdate maxUnavailable 0, HPA/PDB/NetworkPolicy) + Observability-Config (ServiceMonitor/Alerts/Grafana) + pg_dump-Backup-CronJob + Disaster-Recovery-Runbook. Quality-Gate grün; Web-Build erzeugt SW + standalone real. Neu: TD-S16b-01 (Workspace-TS-Precompile), TD-S16b-02 (/api/metrics offen). Schema unverändert. Neue Deps: @ducanh2912/next-pwa (web), sharp (root). |
